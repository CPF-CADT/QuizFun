...
export async function refreshToken(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (!storedToken) {
      console.log(
        `Refresh failed for user ${decodedUser.id}: No token found in Redis. User may have logged out.`
      );
      res.clearCookie("refreshToken", {
        httpOnly: true,
        secure: config.nodeEnv === "production",
    ...
    console.log(
      `Successfully validated refresh token for user ${decodedUser.id}. Issuing new tokens.`
    );
