...
export const GameResultDetails: React.FC<GameResultDetailsProps> = ({ payload, yourUserId, onExit, setSelectedPlayer, isHost }) => {
    ...
    const getPodiumClass = (index: number) => {
        if (!isHost) return 'bg-gradient-to-t from-indigo-500 to-purple-500 border-purple-400 text-white'; // Special style for single player view
        switch(index) {
                        ...
                        return (
                            <li key={p.name} className={`flex items-center justify-between p-3 sm:p-4 rounded-lg transition-all duration-500 ${isRevealed ? 'opacity-100 transform-none' : 'opacity-0 translate-x-4'} ${getPodiumClass(index)}`}>
                                <div className="flex items-center gap-3 sm:gap-4">
                                    {isHost && <div className="font-bold text-lg sm:text-xl w-8 text-center">{index + 1}</div>}
                                    <div className="font-semibold text-base sm:text-lg truncate">{p.name}</div>
                                    {index === 0 && isHost && <Crown className="w-5 h-5 text-yellow-600" />}
                                </div>
                                <div className="flex items-center gap-3 sm:gap-4">
