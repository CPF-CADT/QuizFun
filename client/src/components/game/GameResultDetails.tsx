...
export const GameResultDetails: React.FC<GameResultDetailsProps> = ({ payload, yourUserId, sessionId, onExit, setSelectedPlayer, isHost }) => {
    ...
    useEffect(() => {
        // Reveal players with a slight delay for animation
    ...
    const getPodiumClass = (index: number) => {
        switch(index) {
            case 0: return 'bg-gradient-to-t from-yellow-500 to-yellow-300 border-yellow-400 text-yellow-900';
            case 1: return 'bg-gradient-to-t from-gray-400 to-gray-200 border-gray-300 text-gray-800';
            case 2: return 'bg-gradient-to-t from-orange-500 to-orange-300 border-orange-400 text-orange-900';
    ...
    return (
        <div className="w-full max-w-4xl mx-auto p-4 sm:p-6 bg-gray-800/60 backdrop-blur-lg rounded-2xl shadow-xl border border-white/20 text-white relative overflow-hidden flex flex-col h-full">
            ...
            <div className="relative z-10 flex-grow flex flex-col">
                ...
                {/* Podium for top 3 on larger screens */}
                <div className="hidden md:flex justify-center items-end gap-4 mb-6">
                    {sortedResults.slice(0, 3).map((p, index) => (
                        <div key={p.name} className={`w-1/4 p-4 rounded-t-lg text-center flex flex-col items-center ${getPodiumClass(index)}`}
                             style={{ order: index === 1 ? 0 : (index === 0 ? 1 : 2), height: index === 0 ? '180px' : '150px' }}>
                            <div className="text-2xl font-bold">{index === 0 ? '👑' : `#${index + 1}`}</div>
                ...
                <ul className="space-y-3 flex-grow overflow-y-auto p-2 bg-black/20 rounded-xl">
                    {sortedResults.map((p, index) => {
                        ...
                        return (
                            <li key={p.name} className={`flex items-center justify-between p-3 sm:p-4 rounded-lg transition-all duration-500 ${isRevealed ? 'opacity-100 transform-none' : 'opacity-0 translate-x-4'} ${getPodiumClass(index)}`}>
                                <div className="flex items-center gap-3 sm:gap-4">
                                    <div className="font-bold text-lg sm:text-xl w-8 text-center">{index + 1}</div>
                                    <div className="font-semibold text-base sm:text-lg truncate">{p.name}</div>
                                    {index === 0 && <Crown className="w-5 h-5 text-yellow-600" />}
                                </div>
                                <div className="flex items-center gap-3 sm:gap-4">
                                    <span className="font-bold text-lg sm:text-xl">{p.score} pts</span>
                                    {canViewDetails && (
                                        <button onClick={() => handleViewDetails(p)} className="bg-blue-500/50 hover:bg-blue-500/80 text-white font-semibold px-3 py-1 rounded-md text-xs sm:text-sm transition-colors">
                                            Details
                                        </button>
                                    )}
                                </div>
