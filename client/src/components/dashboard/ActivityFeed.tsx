...
import { reportApi, type IActivitySession, type IActivityFeedResponse } from '../../service/reportApi';
...
const ActivityFeed: React.FC = () => {
  ...
  const fetchActivities = useCallback(async (pageNum: number) => {
    setLoading(true);
    try {
      const response = await reportApi.getUserActivityFeed(pageNum, LIMIT) as { data: IActivityFeedResponse };
        ...
        {activities.map(session =>
          session.role === 'host' ? (
            ...
            <PlayerSessionCard 
              key={session._id} 
              session={session} 
              onViewResults={handleViewPlayerPerformance}
            />
          )
        )}
