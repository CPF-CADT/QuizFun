...
const Sidebar: React.FC<SidebarProps> = ({ activeSection, setActiveSection, sidebarOpen, setSidebarOpen, currentTime }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar - Updated with better mobile width */}
      <div className={`fixed lg:static inset-y-0 left-0 z-50 w-72 lg:w-80 bg-white/80 backdrop-blur-xl border-r border-gray-200/50 flex flex-col shadow-2xl transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        
        {/* Logo Section - Reduced padding on mobile */}
        <div className="p-4 lg:p-8 border-b border-gray-200/50">
          <div className="flex items-center justify-between mb-4 lg:mb-6">
            <div className="flex items-center">
              <div>
                <a href='/'>
                  <h1 className="text-xl lg:text-2xl font-bold bg-gradient-to-r from-violet-600 to-purple-600 bg-clip-text text-transparent">Fun Quiz</h1>
                </a>
                <p className="text-gray-500 text-xs lg:text-sm">Educational Platform</p>
              </div>
            </div>
          ...
          <div className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl lg:rounded-2xl p-3 lg:p-4 mb-4 lg:mb-6 border border-gray-100">
            <div className="flex items-center justify-between mb-2">
              <div>
                <p className="text-gray-900 font-semibold text-sm lg:text-base">{user?.name || 'Guest User'}</p>
              ...
              <div className="text-right">
                ...
                <p className="text-violet-600 text-xs lg:text-sm font-mono">{currentTime.toLocaleTimeString()}</p>
              </div>
            </div>
        ...
        <nav className="flex-1 p-3 lg:p-6 space-y-1 lg:space-y-2">
          {sidebarItems.map((item) => (
            ...
            >
              <div className={`p-2 lg:p-2.5 rounded-lg lg:rounded-xl mr-3 lg:mr-4 transition-all duration-300 ${
                activeSection === item.section 
              ...
              <span className="flex-1 text-left text-sm lg:text-base">{item.name}</span>
              {activeSection === item.section && (
        ...
        {/* Pro upgrade section - Responsive */}
