// src/pages/MyQuizz.tsx (UPDATED & FIXED)

import React, { useState, useEffect, useMemo } from "react";
import { useNavigate } from 'react-router-dom';
import { Search , PlusCircle, Edit, Play, ChevronLeft, ChevronRight, Share2 } from 'lucide-react'; // <-- Import Share2 icon
...
const MyQuizz: React.FC = () => {
    ...
    // --- State for the Solo Link Modal ---
    const [isSoloModalOpen, setSoloModalOpen] = useState(false);
    const [selectedQuizId, setSelectedQuizId] = useState<string | null>(null);

    // --- This function OPENS the solo link modal ---
    ...
    // --- FIX: Add the handleShareSolo function to the actions array ---
    const cardActions: CardAction[] = [
        { label: 'Edit', icon: Edit, onClick: handleEditQuiz, style: 'bg-gray-100 hover:bg-violet-100 text-gray-800' },
        { label: 'Share Solo', icon: Share2, onClick: handleShareSolo, style: 'bg-gray-100 hover:bg-indigo-100 text-gray-800' },
    ...
    const hasPrev = paginationData ? currentPage > 1 : false;
    const hasNext = paginationData ? currentPage < paginationData.totalPages : false;
