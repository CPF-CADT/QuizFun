...
import { ArrowLeft, Eye, Loader, ShieldAlert, Crown, Home, Trophy, Medal } from 'lucide-react';
...
const ResultPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        gameApi.getSessionResults(sessionId, { userId: user._id, view: 'summary' })
            .then(res => {
    ...
    const getRankIcon = (index: number) => {
        switch (index) {
            ...
            case 1: return <Medal className="w-5 h-5 text-gray-500" />;
            case 2: return <Trophy className="w-5 h-5 text-amber-600" />;
            default: return <span className="text-gray-500 font-semibold w-5 h-5 flex items-center justify-center">{index + 1}</span>;
        }
    };
    ...
    const getRankBadge = (index: number) => {
        switch (index) {
            case 0: return <span className="text-sm font-semibold text-yellow-600 px-2 py-0.5 rounded-full bg-yellow-100">Champion</span>;
            case 1: return <span className="text-sm font-semibold text-gray-600 px-2 py-0.5 rounded-full bg-gray-100">Runner-Up</span>;
            case 2: return <span className="text-sm font-semibold text-amber-600 px-2 py-0.5 rounded-full bg-amber-100">Third Place</span>;
            default: return null;
        }
    };
                        ...
                        {sortedResults.length > 0 ? (
                            sortedResults.map((player, index) => (
                                <div
                                    ...
                                    className={`flex items-center justify-between p-4 rounded-lg shadow-sm transition-shadow duration-200 hover:shadow-md ${getRankStyle(index)}`}
                                >
