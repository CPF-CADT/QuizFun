...
const Explore: React.FC = () => {
  ...
  const handleSearch = () => {
    ...
    setCurrentPage(1);
    setQuizzes([]); // Clear previous results
  };
    ...
    const fetchCreatorNames = async () => {
      ...
      try {
        ...
        const newNames = responses.reduce((acc, res) => {
          ...
          if (user?._id && user.name) acc[user._id.toString()] = user.name;
          return acc;
  ...
  // Auto load more quizzes on scroll
  useEffect(() => {
          ...
          <div className="relative bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-xl">
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              <button onClick={() => { setSelectedCategory(''); setQuizzes([]); setCurrentPage(1); }} 
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${!selectedCategory ? 'bg-gradient-to-r from-violet-600 to-purple-600 text-white shadow-lg' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                All Categories
              </button>
              {allCategories.map(category => (
                <button key={category} onClick={() => { setSelectedCategory(category); setQuizzes([]); setCurrentPage(1); }}
                  className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${selectedCategory === category ? 'bg-gradient-to-r from-violet-600 to-purple-600 text-white shadow-lg' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                  {category}
                </button>
              ))}
            </div>
            ...
            <>
              ...
              {hasNextPage && (
                <div ref={sentinelRef} className="mt-12 text-center">
                  {loadingMore && (
                    <Loader className="w-8 h-8 text-violet-600 animate-spin mx-auto" />
                  )}
                </div>
              )}
            </>
